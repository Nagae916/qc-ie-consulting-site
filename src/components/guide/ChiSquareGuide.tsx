...
export default function ChiSquareGuide() {
  // 観測度数（固定サンプル）
  const [obs] = useState<number[][]>([[30, 20], [20, 30]]); // 2×2
  const rowTotals = useMemo(() => obs.map(r => r.reduce((s, v) => s + v, 0)), [obs]); // [50,50]
  const colTotals = useMemo(
    () => [0, 1].map(j => obs[0][j] + obs[1][j]),
    [obs]
  ); // [50,50]
  const grandTotal = useMemo(() => rowTotals.reduce((s, v) => s + v, 0), [rowTotals]); // 100
  ...
  const handleCalcExpected = () => {
    ...
    for (let i = 0; i < 2; i++) {
      for (let j = 0; j < 2; j++) {
        e[i][j] = (rowTotals[i] * colTotals[j]) / grandTotal;
      }
    }
    setExpected(e);
