...
MIP、APS、MES、TOC／DBR、SMED、SCM、S&OPなど、生産管理・IEの既存Knowledgeは、現在のlegacy guide taxonomyである `content/guides/engineer/` と `/guides/engineer/` に配置されている。GTも同じ既存パターンに合わせ、`/guides/engineer/group-technology` を維持する。

このURLは現行の配置規則に従うものであり、GT本文そのものを技術士専用Knowledgeとして位置付けるものではない。「生産を整える」テーマハブと関連する通常Knowledgeから到達できるようにする。将来Information Architectureを再整理する場合は移行候補になり得るが、URL変更はredirect、canonical、内部リンク、sitemapを含む別タスクとして判断する。
...
- 前後空白だけを除去して照合する。
- ASCIIの大文字・小文字、全角・半角、`&`、ハイフンは機械的に同一視しない。
- 日本語名、英語名、略語は、確認したものだけを `aliases` に明示する。
- URLが空または未設定の正規語は、タグ表示だけに留めてリンクを生成しない。
- 対応表にない語を類似語へfallbackしない。

過剰な正規化を避け、例えば別概念の略語や近い表記を誤って同じKnowledgeへ結び付けないことを優先する。正規語・別名の重複、別名衝突、正本URLの実在、代表語の解決、未知語の非解決は自動監査で確認する。
