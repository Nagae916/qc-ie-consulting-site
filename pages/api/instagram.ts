...
function readEnv() {
  ...
  const limit = Number.isFinite(limitEnv) && limitEnv > 0 && limitEnv <= 12 ? limitEnv : 3;
...
function extractGraphError(raw: string): string {
  try {
    ...
    const hint = e.code === 190 ? "（アクセストークンの期限切れ/無効の可能性）" : "";
    return `Instagram API error: code=${e.code ?? "?"} type=${e.type ?? "?"} ${e.message}${hint}`;
  } catch {
    return raw;
  }
}
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // フロントを落とさない方針：エラーでも 200 & 空配列、ヘッダに詳細
  const safeEnd = (statusHint: string, message?: string) => {
    ...
    return res.status(200).json({ items: [] as IgItem[] });
  };
  ...
  try {
    const qLimitRaw = Array.isArray(req.query.limit) ? req.query.limit[0] : req.query.limit;
    const qLimitNum = Number(qLimitRaw);
    const finalLimit =
      Number.isFinite(qLimitNum) && qLimitNum > 0 && qLimitNum <= 12 ? qLimitNum : limit;
